...
int ngx_http_lua_config_file_is_exist(ngx_str_t *filename) {
    u_char  dst[NGX_MAX_PATH];
    memset(dst,0,sizeof(u_char)*NGX_MAX_PATH);
    int n = ngx_http_script_variables_count(filename);
    if (n>0) {
        return -1; //文件路径存在变量
    }
    if (filename->data[0] =='/') {
        ngx_memcpy(dst, filename->data,filename->len);
    }
    else {
        ngx_snprintf(dst,sizeof(NGX_PREFIX)+filename->len,"%s%s",NGX_PREFIX,filename->data);
    }
    if ( access((const char *)dst,0) == 0 ){
        return 0; //文件存在
    }
    return -2;//文件不存在
}
