...
static int ngx_http_lua_ngx_sha512(lua_State *L);
...
ngx_http_lua_inject_string_api(lua_State *L)
{
    ...
    lua_pushcfunction(L, ngx_http_lua_ngx_sha512);
...
static int
ngx_http_lua_ngx_sha512(lua_State *L)
{
    u_char                  *src;
    size_t                   slen;

    //ngx_sha512_context       sha;
    u_char                   sha_buf[SHA512_DIGEST_LENGTH];
    ...
    dd("sha512len: %d", (int) slen);

    ngx_sha512(src, slen, sha_buf, 0);
    
    ngx_hex_dump(hex_buf, sha_buf, sizeof(sha_buf));
    lua_pushlstring(L, (char *) hex_buf, sizeof(hex_buf));
