...
ngx_http_upstream_check_http_init(ngx_http_upstream_check_peer_t *peer)
{
	...
	///add by zdw
	u_char ipstr[100] = "";
	ngx_sock_ntop(peer->peer_addr->sockaddr,sizeof(ngx_sockaddr_t), ipstr,100,0);

	u_char *last ;
	ngx_str_t header;
	ngx_uint_t  len = ucscf->send.len + sizeof("Host: ") -1 + ngx_strlen(ipstr) + sizeof("\r\n\r\n")-1;
	header.len = len;
	header.data = ngx_palloc(peer->pool, len);
	if(header.data == NULL) {
		return NGX_ERROR;
	}
	last = header.data;
	last = ngx_cpymem(last, ucscf->send.data, ucscf->send.len);
	last = ngx_cpymem(last, "Host: ", sizeof("Host: ") - 1);
	last = ngx_cpymem(last, ipstr, ngx_strlen(ipstr));
	last = ngx_cpymem(last, "\r\n\r\n", sizeof("\r\n\r\n")-1);
	ctx->send.start = ctx->send.pos = header.data;
	ctx->send.end = ctx->send.last = header.data + header.len;
	///
