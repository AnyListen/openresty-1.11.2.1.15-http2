...
static ngx_uint_t ngx_http_v2_check(ngx_event_t *rev);
...
static ngx_uint_t ngx_http_v2_check(ngx_event_t *rev)
{
	...
	if(c->ssl && c->ssl->handshaked == 1){
		return 1;
	}

	size = hc->proxy_protocol ? sizeof(buf) : 2;
	...
	if (hc->proxy_protocol) {
		...
		c->log->action = "check http2 or http1";
