		...
		ngx_http_upstream_srv_conf_t *us)
{
    ...
    if(r->backupos_request == 1){
	rrp->backupos_request = 1;
    }
...
ngx_http_upstream_get_round_robin_peer(ngx_peer_connection_t *pc, void *data)
{
    ...
    if (peers->single) {
		...
		//add lhc  302 dispatch check server is in gslb_white_list;
		if(rrp->backupos_request){
			len_ip = peer->name.len;	
			if((ch = ngx_strlchr(peer->name.data, peer->name.data + peer->name.len, ':')) != NULL){
				len_ip = ch - peer->name.data;
			}

			if(ngx_http_gslb_check_peer_down(ntohl(ngx_inet_addr(peer->name.data, len_ip)))){
				goto failed;
			}
		}
		rrp->current = peer;
