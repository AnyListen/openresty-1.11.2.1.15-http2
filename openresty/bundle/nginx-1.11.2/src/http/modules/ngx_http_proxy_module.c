...
typedef struct {
	...
	//add by lhc
    ngx_array_t                   *redirect_uri_lengths;
    ngx_array_t                   *redirect_uri_values;
...
static char *ngx_http_proxy_redirect(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
static char *ngx_http_proxy_redirect_uri(ngx_conf_t *cf, ngx_command_t *cmd, 
	void *conf);
...
static ngx_command_t  ngx_http_proxy_commands[] = {
    ...
    { ngx_string("proxy_redirect_uri"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
      ngx_http_proxy_redirect_uri,
      NGX_HTTP_LOC_CONF_OFFSET,
      0,
      NULL },
...
ngx_http_proxy_create_request(ngx_http_request_t *r)
{
	...
	u_char				 *last, *last_tmp, *req_pos, *and_pos, *start_pos, *end_pos;
    ...
    //add by lhc
    if(plcf->redirect_uri_lengths){
		if( ngx_http_script_run(r, &redirect_uri, plcf->redirect_uri_lengths->elts, 0, plcf->redirect_uri_values->elts) == NULL )
		{
			return NGX_ERROR;
		}
		uri_len = redirect_uri.len;
    } else if (plcf->proxy_lengths && ctx->vars.uri.len) {
    ...
    *b->last++ = ' ';
    ...
    //add by lhc
    if(plcf->redirect_uri_lengths) {
		b->last = ngx_copy(b->last, redirect_uri.data, redirect_uri.len);
...
ngx_http_proxy_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
	...
	//add by lhc
    if (conf->redirect_uri_lengths == NULL) {
		conf->redirect_uri_lengths = prev->redirect_uri_lengths;
		conf->redirect_uri_values = prev->redirect_uri_values;
    }
...
static char *
ngx_http_proxy_redirect_uri(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
	ngx_uint_t                  n = 0;
	ngx_str_t			*value, *url;
	ngx_http_script_compile_t   sc;	
	...
	if(n){
		sc.cf = cf;
		sc.source = url;
		sc.lengths = &plcf->redirect_uri_lengths;
		sc.values = &plcf->redirect_uri_values;		
		sc.complete_lengths = 1;
		sc.complete_values = 1;
...
ngx_str_t *ngx_http_proxy_get_upstream(ngx_http_request_t *r)
{
	...
	host = ngx_pcalloc(r->pool, sizeof(ngx_str_t));
	if(host == NULL){
		return NULL;
	}
	...
	if (plcf->proxy_lengths != NULL) {
		...
		if (proxy.len > 7
				&& ngx_strncasecmp(proxy.data, (u_char *) "http://", 7) == 0)
		{
			host->len = proxy.len - 7;
			host->data = ngx_pcalloc(r->pool, host->len);
			if(host->data == NULL){
				return NULL;
			}

			ngx_memcpy(host->data, proxy.data + 7, host->len);
#if (NGX_HTTP_SSL)
		}else if (proxy.len > 8
				&& ngx_strncasecmp(proxy.data, (u_char *) "https://", 8) == 0)
		{
			host->len = proxy.len - 8;
			host->data = ngx_pcalloc(r->pool, host->len);
			if(host->data == NULL){
				return NULL;
			}

			ngx_memcpy(host->data, proxy.data + 8, host->len);
#endif
		}
		else{
			return NULL;
		...
		host = &plcf->upstream.upstream->host;
	}
