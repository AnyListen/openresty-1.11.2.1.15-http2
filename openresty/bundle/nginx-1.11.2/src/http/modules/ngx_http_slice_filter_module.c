...
typedef struct {
	ngx_str_t				name;
	ngx_array_t				*pcs;		// ngx_peer_connection_t;
}ngx_http_slice_upstream_peers_t;
...
static void ngx_http_slice_connect_handler(ngx_peer_connection_t *pc);
static ngx_int_t ngx_http_slice_peek_one_byte(ngx_connection_t *c);
static void ngx_http_slice_purge_send_handler(ngx_event_t *event);
static void ngx_http_slice_purge_recv_handler(ngx_event_t *event);
...
ngx_http_slice_header_filter(ngx_http_request_t *r)
{
	...
	ngx_http_slice_upstream_peers_t *up, *upp;
    ...
    if (ctx->etag.len) {
               ...
               != 0)
        {
			...
			up = NULL;
			if(r->upstream) {
				us = r->upstream->conf->upstream;
				slcf = ngx_http_get_module_loc_conf(r, ngx_http_slice_filter_module);

				upp = slcf->ups->elts;
				for(i = 0; i < slcf->ups->nelts; i++) {
					if(upp[i].name.len == us->host.len && ngx_strncmp(upp[i].name.data, us->host.data, us->host.len) == 0) {
						up = &upp[i];
						break;
					}
				}
				...
				if( i == slcf->ups->nelts) {
					up = ngx_array_push(slcf->ups);
					if(up == NULL) {
						return NGX_ERROR;
					}
					
					up->name = us->host;
					up->pcs = ngx_array_create(ngx_cycle->pool, us->servers->nelts, sizeof(ngx_peer_connection_t));
					if(up->pcs == NULL) {
						return NGX_ERROR;
					}
					...
					for( i = 0 ; i < us->servers->nelts; i++) {
						pc = ngx_array_push(up->pcs);
						pc->sockaddr = server[i].addrs[0].sockaddr;
						pc->socklen = server[i].addrs[0].socklen;
						pc->name = &server[i].addrs[0].name;
						...
						b = ngx_create_temp_buf(ngx_cycle->pool, 512);
						if(b == NULL) {
							return NGX_ERROR;
						}
				...
				//init request
				static ngx_str_t fix1 = ngx_string("DIR_PURGE http://");
				static ngx_str_t fix2 = ngx_string(" HTTP/1.0\r\nHost: ");
				static ngx_str_t fix3 = ngx_string("\r\nUser-Agent: Dnion-UA-nginx-purge\r\n\r\n");

				buf_size = fix1.len + r->headers_in.server.len + r->uri.len + fix2.len + r->headers_in.server.len + fix3.len;
				...
				for(i = 0; i < up->pcs->nelts; i++) {
					...
					//connect upstream
					ngx_http_slice_connect_handler(&pc[i]);
				}
...
static void 
ngx_http_slice_connect_handler(ngx_peer_connection_t *pc)
{
	ngx_int_t	rc;
	ngx_connection_t		*c;
	...
	if(pc->connection != NULL) {
		c = pc->connection;
		if ((rc = ngx_http_slice_peek_one_byte(c)) == NGX_OK) {
			return;
		} else {
	...
	if (rc == NGX_ERROR || rc == NGX_DECLINED) {
		ngx_log_error(NGX_LOG_ERR, pc->log, ngx_socket_errno, "connect server fail");
		return;
	}
	...
	c->write->handler = ngx_http_slice_purge_send_handler;
	c->read->handler = ngx_http_slice_purge_recv_handler;
	return ;
}

static void
ngx_http_slice_purge_send_handler(ngx_event_t *event)
{
	size_t					size;
	ngx_buf_t				*b;
	ngx_connection_t		*c;
...
static void
ngx_http_slice_purge_recv_handler(ngx_event_t *event)
{
	return ;
}

static ngx_int_t
ngx_http_slice_peek_one_byte(ngx_connection_t *c)
{
	char                            buf[1];
	ngx_int_t                       n;
	ngx_err_t                       err;
...
ngx_http_slice_create_loc_conf(ngx_conf_t *cf)
{
	...
	slcf->ups = ngx_array_create(cf->pool, 8, sizeof(ngx_http_slice_upstream_peers_t));
	if(slcf->ups == NULL) {
		return NULL;
	}
