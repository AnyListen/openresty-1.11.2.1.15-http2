...
static ngx_int_t ngx_http_variable_crc(ngx_http_request_t *r, 
		ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_variable_pqsi(ngx_http_request_t *r,
	   	ngx_http_variable_value_t *v, uintptr_t data);

static ngx_int_t ngx_http_variable_gslb(ngx_http_request_t *r,
        ngx_http_variable_value_t *v, uintptr_t data);

static ngx_int_t ngx_http_variable_node(ngx_http_request_t *r,
        ngx_http_variable_value_t *v, uintptr_t data);
...
static ngx_http_variable_t  ngx_http_core_variables[] = {
    ...
    //add by zdw
    { ngx_string("crc"), NULL, ngx_http_variable_crc,
    			0, 0, 0 },
    { ngx_string("pqsi"), NULL, ngx_http_variable_pqsi,
   	0, 0, 0 },
    //

    //add by lhc
	{ ngx_string("gslb"), NULL, ngx_http_variable_gslb,
		0, 0, 0 },
    { ngx_string("node"), NULL, ngx_http_variable_node,
		0, 0, 0 },
    //end add
    ...
    ngx_http_variable_value_t *v, uintptr_t data)
{
	...
	if(r->request_id.len == 32){
                v->valid = 1;
                v->no_cacheable = 0;
                v->not_found = 0;
                v->len = 32;
                v->data = r->request_id.data;
                return NGX_OK;
        }
	...
	for(i = 0; /* void */; i++){
		...
		if( header[i].key.len == ngx_strlen("X-Request-ID")
			&& ngx_strncmp("X-Request-ID", header[i].key.data, header[i].key.len) == 0)
		{
			v->valid = 1;
			v->no_cacheable = 0;
			v->not_found = 0;
			...
			r->request_id.len = header[i].value.len;
                	r->request_id.data = header[i].value.data;
    ...
    if (RAND_bytes(random_bytes, 16) == 1) {
		...
		r->request_id.len = v->len;
		r->request_id.data = v->data;
        return NGX_OK;
    }
...
static ngx_int_t
ngx_http_variable_gslb(ngx_http_request_t *r, ngx_http_variable_value_t *v,
        uintptr_t data)
{
	if(r->gslb_dispatch) {
		v->data = (u_char *)"1";
...
static ngx_int_t 
ngx_http_variable_crc(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data)
{
	ngx_uint_t i;
	...
	for (i = 0; /* void */ ; i++) {
		...
		if (tb[i].key.len == 10 && ngx_strncmp("VcLogToNgx", tb[i].key.data, 10) == 0) {

			e = tb[i].value.data + tb[i].value.len;
			p1 = tb[i].value.data;

			while(*p1 && isspace(*p1) && p1 < e) p1++;
			p2 = p1;
			while(*p2 && !isspace(*p2) && *p2 != ',' && p2 < e) p2 ++;
...
static ngx_int_t 
ngx_http_variable_pqsi(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data)
{
	ngx_uint_t i;
	ngx_http_upstream_t *u;
	ngx_list_part_t  *part;
...
static ngx_int_t
ngx_http_variable_node(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{
    u_char			*s, *e, *p1, *p2,*p3,*first,*last;

    s = ngx_cycle->hostname.data;
    e = ngx_cycle->hostname.data + ngx_cycle->hostname.len;
