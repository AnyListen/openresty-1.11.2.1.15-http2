...
ngx_http_create_request(ngx_connection_t *c)
{
	...
	//初始化limit_msec
	r->limit_msec = tp->sec * 1000 + tp->msec;
	//end
    ...
    ngx_uint_t offset)
{
    ...
    //add by lhc 二次调度重组
    in_addr_t  ip_addr;
    ngx_str_t  new_host,host_modify;
    u_char     *ch0, *ch1, *ch2;

    ngx_str_null(&new_host);
    ngx_str_null(&host_modify);

    ch0 = ngx_strlchr(h->value.data, h->value.data + h->value.len, ':'); 
    if(ch0 != NULL){
		host_modify.data = h->value.data;
		host_modify.len = ch0 - h->value.data;
    }
    else{
		host_modify = h->value;
    }

    ip_addr = ngx_inet_addr(host_modify.data, host_modify.len);
    if( ip_addr != INADDR_NONE ){
		new_host.data = r->uri.data + 1;
		ch1 = ngx_strlchr(r->uri.data + 1, r->uri.data + r->uri.len, '/');
		ch2 = ngx_strlchr(r->unparsed_uri.data + 1, r->unparsed_uri.data + r->unparsed_uri.len, '/');
		if(ch1 != NULL && ch2 != NULL){
			new_host.len = ch1 - (r->uri.data + 1);
			h->value = new_host;

			//reset uri and unparsed_uri
			r->uri.len = r->uri.len - (ch1 - r->uri.data);
			r->uri.data = ch1;
			r->unparsed_uri.len = r->unparsed_uri.len - (ch2 - r->unparsed_uri.data);
			r->unparsed_uri.data = ch2;
