...
#include <ngx_core.h>
#include <ngx_sha512.h>


#define UL64(x) x##ULL

/* Implementation that should never be optimized out by the compiler */
static void ngx_zeroize( void *v, size_t n ) {
    volatile u_char *p = v; while( n-- ) *p++ = 0;
}
...
{
    memset( ctx, 0, sizeof( ngx_sha512_context ) );
}
...
void ngx_sha512_free( ngx_sha512_context *ctx )
{
    ...
    ngx_zeroize( ctx, sizeof( ngx_sha512_context ) );
}
                    ...
                    size_t ilen )
{
    ...
    if( left && ilen >= fill )
    {
        ...
        ngx_sha512_process( ctx, ctx->buffer );
        input += fill;
        ilen  -= fill;
        left = 0;
    }
...
static const u_char sha512_padding[128] =
{
...
void ngx_sha512_finish( ngx_sha512_context *ctx, u_char output[64] )
{
    ...
    ngx_sha512_update( ctx, sha512_padding, padn );
    ngx_sha512_update( ctx, msglen, 16 );
...
{
    ngx_sha512_context ctx;

    ngx_sha512_init( &ctx );
    ngx_sha512_starts( &ctx, is384 );
    ngx_sha512_update( &ctx, input, ilen );
    ngx_sha512_finish( &ctx, output );
    ngx_sha512_free( &ctx );
}
