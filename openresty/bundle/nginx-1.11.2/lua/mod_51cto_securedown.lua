local ok,info = pcall(require, "mod_51cto_securedown_conf")
local _M =
{
}
...
function _M.anti_stealing_link()
	if not ok or type(info) ~= "table" then
	   ngx.log(ngx.ERR,"not find config: mod_51cto_securedown_conf.lua"," or config format err(",info,")")
	   return
	end
	--PURGE方法忽略
	...
	local des = uri_args["skey"] 
	local time_stamp = uri_args["stime"]
	local sname = uri_args["sname"]
	
	if not path or not des or not time_stamp or not sname then
	   ngx.log(ngx.INFO, "Invalid Param:",ngx.var.args)
	   return go_exit(fin_conf)
	end 
	--验证时间是否过期
	...
	--验证加密字符串
	local buf = fin_conf.passwd ..path .. time_stamp
	...
	if string.lower(fin_str) == string.lower(des) then
	   ngx.req.set_uri(path); --此处是否需要,之前此处由http_request_rewrite插件实现更改url
	   uri_args["path"] = nil 
