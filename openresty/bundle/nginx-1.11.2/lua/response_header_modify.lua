...
local ok,info = pcall (require, "response_header_conf")
function _M.modify_header()
	if not ok or type(info) ~= "table" then
	  ngx.log(ngx.ERR,"not find config: response_header_conf.lua"," or config format err")
	  return
	end
	...
	local conf = cf.response_header
	if conf == nil then
	  ngx.log(ngx.INFO,"no response_header config,"," please check")
	  return
	end
	for i=1,#conf do
	  local header = conf[i].header
	  if header then
	    local header_value=ngx.resp.get_headers()[header]
	    if header_value then
	      ...
	      if conf[i].value == nil then
	        ...
	        elseif at_s == 1 then
	          ...
	          header_value = header_value ..";" .. temp 
	        elseif at_s == len then
	          if client_ip then 
	        ...
	        ngx.header[header] = header_value;
	      end
	    else
